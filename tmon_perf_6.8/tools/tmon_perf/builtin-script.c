...
#include "../../../postmortem_analyzer/common.h"
...
__attribute__((unused)) static void process_event(struct perf_script *script,
...
// This function is part of our TMon-Perf API.
// It delivers ptwrite events to TMon postmortem analyzer.
static void tmon_deliver_event(struct perf_sample *sample)
{
	struct synth_intel_ptwrite ptwr_synth;
	struct perf_synth_intel_ptwrite *data = perf_sample__synth_ptr(sample);

	ptwr_synth.pid = sample->pid;
	ptwr_synth.tid = sample->tid;
	ptwr_synth.payload = le64_to_cpu(data->payload);
	
	tmon_pass_event(&ptwr_synth);
}
